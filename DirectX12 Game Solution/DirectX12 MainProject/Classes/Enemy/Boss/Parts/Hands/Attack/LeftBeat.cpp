...
void boss::LeftBeat::HandCheck(const ObjectManager* const obj_m) {	//手の状態を確認
...
void boss::LeftBeat::Ready(const ObjectManager* const obj_m) { //プレイヤーの座標に手を移動させる
...
void boss::LeftBeat::LeftBeatAttack(ActionManager* const act_m) { //叩きつけ攻撃
...
void boss::LeftBeat::Wait(ActionManager* const act_m) {	//硬直
...
void boss::LeftBeat::HandReturn() { //手を初期位置へ移動
