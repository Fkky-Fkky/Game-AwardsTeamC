...
void RightBeat::HandReturn() {
	...
	if (wait_time >= 2.0f) {
		if (pos.x < INITIAL_POS_X)
			pos.x = std::min(pos.x + 10.0f * time_delta, INITIAL_POS_X);
		else
			pos.x = std::max(pos.x - 10.0f * time_delta, INITIAL_POS_X);

		pos.y = std::min(pos.y + 5.0f * time_delta, INITIAL_POS_Y);
		rote.x = std::min(rote.x + 1.0f * time_delta, XM_PIDIV4);
	}

	if (pos.y >= INITIAL_POS_Y && pos.x == INITIAL_POS_X) {
		wait_time = 0.0f;
		beat_time = 0.0f;
		getposflag = false;
