...
void boss::Death::Update(const float deltaTime, const ObjectManager* const obj_m, HandManager* const hand_m) {
	...
	case DEATH_ACTION:	DeathAction(hand_m);		break;
	case ACTION_END:
		hand_m->DeathActionEnd();
		hand_m->SetVerticalShake(false);			break;
	}
...
void boss::Death::DeathAction(HandManager* const hand_m) {	//‚â‚ç‚ê‚½Žž‚Ì“®‚«
